...
module.exports = {
  ...
  serialize () {
    ...
    this.caches.set(projectPath, debugModel.serialize())

    let serializeData = {caches: this.caches, version: serializeVer}
  ...
  deserialize ({data}) {
    ...
    if (data.version && new RegExp(reg).exec(data.version)[1] === new RegExp(reg).exec(serializeVer)[1]) {
      ...
      let debugModelCache = this.caches.get(atom.project.getDirectories()[0].path)
      debugModel.deserialize(debugModelCache)
