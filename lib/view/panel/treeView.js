...
class TreeView extends React.Component {
  ...
  _createRow (node, level) {
    ...
    const nodeIcon = node.leaf ? icon
      : React.createElement("a", {onClick: this._nodeClick.bind(this, node)}, 
        icon
      )
    const nodeRow = (
      React.createElement("div", {style: {marginLeft: level * this.props.indent + 'px'}}, 
        nodeIcon, 
        content
      )
    )
  ...
  _resolveIcon (node) {
    ...
    if (typeof icon === 'string') {
      ...
      icon = React.createElement("i", {className: className})
    }
